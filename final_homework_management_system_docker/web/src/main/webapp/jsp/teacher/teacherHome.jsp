...
<%--    <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="edit">编辑</a>--%>
    <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="delete">移除班级</a>
</script>
<table id="demo" lay-filter="parse-table-demo">
    <thead>
    <tr>
        <th lay-data="{field: 'sid', width:160, sort: true, fixed: 'left'}">SID</th>
        <th lay-data="{field: 'title', title: '名字', width:180}"></th>
...
<script>
    layui.use(['table', 'jquery'], function(){
        ...
        table.on('toolbar(parse-table-demo)', function(obj){
            ...
            switch(obj.event){
                case 'add':
                    location.href="${pageContext.request.contextPath}/teacher/new-student?tid=<%=String.valueOf(teacher.getTid())%>";
                    break;
            }
        });
        //监听行工具事件
        table.on('tool(parse-table-demo)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){
                ...
                layer.confirm('确定删除吗?', {btn: ['确定删除', '取消']}, function (index) {
                    ...
                    $.ajax({
                        url: "${pageContext.request.contextPath }/teacher/student",
                        type: "delete",
                        traditional: true,
                        async: true,
                        data:{
